...
socket.on("recieve", (message) => {
  if (message.length > 2 && typeof message === "object") {
    message.forEach((element) => {
      $("#messages").append(
        `${element[0]}: <span class="text-secondary">${element[1]}</span><br>`
      );
    });
    scrollDown();
  } else {
    $("#messages").append(
      `${message[0]}: <span class="text-secondary">${message[1]}</span><br>`
    );
    scrollDown();
  }
});
...
function sendMessage() {
  ...
  if (message === "/version") {
    ...
    if (navigator.cookieEnabled) {
      ...
      $("#messages").append(
        "Server: You must enable cookies to change your name. Currently, you have cookies disabled, but you can learn how to enable them <a class='text-info' href='https://support.google.com/accounts/answer/61416?hl=en&co=GENIE.Platform%3DDesktop'>here</a><br>"
      );
      resetInput();
    }
    scrollDown();
    ...
    document.getElementById(
      "messages"
    ).innerHTML += `Server: Available commands - /version, /setname, /help<br>/version: Outputs the current version of Minichat you are running<br>/setname: Sets your name to the name you specify, use like /setname <name><br>/help: Outputs the available commands and what they do`;
    resetInput();
    scrollDown();
  } else if (message === "/os") {
    $("#messages").append(
      `Server: You are currently running ${platform.os}<br>`
    );
    resetInput();
    scrollDown();
  } else {
    $.get("/json/emojis.json", (data, status) => {
      ...
      //if the cookie is not set, send the message as anonymous
        ...
        $("#messages").append(
          "Server: <span class='text-danger'>Invalid message. You are not permitted to use html in your message.</span>"
        );
      }
    });
  }
...
//append the welcome message after a 500 ms timeout
setTimeout(() => {
  if ($("#messages").text() != "Messages loading...") {
    $("#messages").append(
      'Server: Welcome to minichat <span class="text-secondary">v1.0.0</span>! For more information, type <span class="text-info">/help</span> in the chat.<br>'
    );
    scrollDown();
  }
}, 500);
